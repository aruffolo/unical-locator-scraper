...
from .transform.normalize import normalize_teachers, write_json
...
def crawl_teachers_command(
    ...
    write_json(sources_file, [source_entry])
...
@click.option("--base-url", required=True, help="Departments page URL.")
def crawl_departments_command(base_url: str) -> None:
    """Placeholder departments crawl command (MVP skeleton)."""
    departments = crawl_departments(base_url=base_url)
    click.echo(f"Found {len(departments)} departments (skeleton)")
